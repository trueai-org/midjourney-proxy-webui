...
import { createIllegalWord, deleteIllegalWord, queryIllegalWord } from '@/services/mj/api';
...
const UserList: React.FC = () => {
  ...
  const handleAdd = async (values: Record<any, any>) => {
    ...
    const res = await createIllegalWord(values);
    if (res.success) {
      api.success({
        message: 'success',
  ...
  const onDelete = async (id: string) => {
    try {
      const res = await deleteIllegalWord(id);
      if (res.success) {
        api.success({
          message: 'success',
  ...
  const columns = [
    {
      title: intl.formatMessage({ id: 'pages.word.name' }),
      dataIndex: 'name',
      width: 160,
      align: 'left',
      fixed: 'left',
    ...
    {
      title: intl.formatMessage({ id: 'pages.word.description' }),
      dataIndex: 'description',
      width: 160,
      align: 'left',
    },
    {
      title: intl.formatMessage({ id: 'pages.word.keywords' }),
      dataIndex: 'keywords',
      width: 120,
      align: 'left',
      // 弹出显示纯文本
      render: (text, record) => {
        return <span>{record?.keywords.length || '0'} {intl.formatMessage({ id: 'pages.word.keywords' })}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'pages.word.enable' }),
      dataIndex: 'enable',
      width: 80,
      showInfo: false,
      hideInSearch: true,
    ...
    {
      title: intl.formatMessage({ id: 'pages.word.weight' }),
      dataIndex: 'weight',
      width: 80,
      align: 'left',
      hideInSearch: true,
    ...
    {
      title: intl.formatMessage({ id: 'pages.word.createTime' }),
    ...
    {
      title: intl.formatMessage({ id: 'pages.word.updateTime' }),
        ...
        return (
          <Space>
            <Button
              ...
              onClick={() =>
                openModal(
                  intl.formatMessage({ id: 'pages.word.update' }),
            ...
            />
            <Popconfirm
              title={intl.formatMessage({ id: 'pages.word.delete' })}
              description={intl.formatMessage({ id: 'pages.word.deleteTitle' })}
        ...
        <ProTable
          ...
          toolBarRender={() => [
            <Button
              ...
              onClick={() =>
                openModal(
                  intl.formatMessage({ id: 'pages.word.add' }),
          ...
          request={async (params) => {
            ...
            const res = await queryIllegalWord({ ...params, pageNumber: params.current! - 1 });
            return {
              data: res.list,
